...
parser.add_argument('--datasize', type=int, default=240, help='Maximum number of images per class')
parser.add_argument('--classes', nargs='+', type=str, default=["none"], help='Available classes')
...
from screwCropperRandom import screwCropper

Cropper = screwCropper(new_size=new_size, )

# Crop screw images for training

if os.path.exists(path_to_screws + 'screws_dict_none.csv'):
  os.remove(path_to_screws + 'screws_dict_none.csv')

with open(path_to_screws + 'screws_dict_none.csv', 'w', newline='') as csvfile:
